              ...
              {[...activeByCinema!.entries()].map(([cinemaId, list]) => {
                ...
                return (
                  <div key={cinemaId} className="hkm-panel rounded-2xl p-4">
                     ...
                     *
                     * ★ 2026-09-26 用户：「手续费的文字和布局现在看着突兀不协调」。
                     *   原本这里是一行裸文字（「$10 手續費」/「— 手續費未確認」）
                     *   夹在戏院地址与场次网格之间，没有容器，
                     *   跟页面上其余药丸标签不是同一种东西。
                     *   现改为药丸（.hkm-fee），字号与颜色由该 class 统一给，
                     *   因此这里只传外距 mb-3，不再传 text-xs / leading-relaxed /
                     *   text-fg —— 那会让三处出现各自长成不同大小。
                     *
                     *   ★ 药丸是 inline-flex，单独一行即可，不需要 leading-relaxed。
                     *
                     *   0 元同样显示（手續費 $0）—— 显式告诉用户「这里不额外收钱」
                     *   比留空更有信息量，也正是用户要的。
                     *
                     *   title 写在整颗药丸上，滑过任一处都能看到说明：
                     *   写明 0 元的来历（会员豁免）与是否已含在票价内，
                     *   避免用户到付款页才发现口径不同。
                     */}
                    <FeeLine
                      amount={c.cinemaFee}
