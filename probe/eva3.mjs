import { resolveImdbIds, imdbRatings } from '../scrapers/imdb.js';
const cases = [
...
for (const [qs, y] of cases) {
  ...
  for (const c of cands.slice(0, 4)) {
    ...
    console.log('   ', c.id, 's=' + c.s, String(c.year).padEnd(6), (c.relaxedYear ? 'relaxed ' : '        ') + JSON.stringify(c.title).padEnd(48), '→', r ? r.rating + ' (' + r.votes + ')' : 'null');
