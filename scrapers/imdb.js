 ...
 * 「这是旧片重映」的豆瓣年份差下限
 *
 * 豆瓣存的是**原作首映年**，重映片会比港映早很多（EVA 1997 → 港映 2026，差 29），
 * 而同名新片基本同年（差 0）。取 5 年是为了放过跨年上映（12 月拍完次年 1 月开画）。
 */
export const REISSUE_MIN_DOUBAN_GAP = 5;

/**
 * 有没有「这是旧片重映」的正面证据
 *
 * ★ 为什么必须有证据才允许回退（2026-09-19 修）
 *
 * 回退规则的原意：重映片在 IMDb 常有 1997 原版（有分）+ 2025 重映条目（无分），
 * 首选无分时改用原版的分数。
 *
 * 但「首选无分」有两种成因，在候选列表里**完全同形**，只靠候选分不开：
 *   a) 真重映：首选是重映新条目（无分），原版有分 → 该回退
 *   b) 同名新片：首选是全新电影（无分），另有同名旧片（有分）→ 绝不能回退
 * 实测 11 个回退里 7 个属于 (b)：生化危機（2026 新片）挂上了 2002 年版的 6.6 分。
 *
 * 判据用豆瓣年份 —— 它是唯一能分开这两种情况的外部信息。
 * 没有豆瓣数据时一律不回退：宁可显示「暫無評分」，
 * 也不能把别的片的分数挂上去（错配比缺数据严重）。
 */
export function isReissueEvidence(doubanYear, hkYear) {
  if (!doubanYear || !hkYear) return false;
  return hkYear - doubanYear >= REISSUE_MIN_DOUBAN_GAP;
}

/**
 * 豆瓣原作年与 IMDb 条目年的允许偏差
 *
 * 实测口径（2026-09-19，211 部全量）：**真匹配的偏差恒为 0**，
 * 而误配的最小偏差是 4（逆權司機：豆瓣 2013「辩护人」vs IMDb 2017「A Taxi Driver」）。
 * 取 1 是给「影展首映年 vs 正式公映年」留最小余量；实测没有任何真重映需要 >0，
 * 而离误配区（≥4）已经很远 —— 宁可漏配显示「暫無評分」，也不挂错分。
 */
export const REISSUE_YEAR_TOLERANCE = 1;

/**
 * 第二道闸门：候选条目的年份与豆瓣原作年是否对得上
 *
 * 第一道闸门只能证明「这是部老片」，不能证明「回退到的这条 IMDb 条目就是它」。
 * 反例（实测）：
 *   恨世者  豆瓣 2017 / 回退 tt0807755:2001「The Misanthrope」→ 差 16 年，是别的改编
 *   天鵝湖  豆瓣 1998 / 回退 tt32986839:2024「Swan Lake」      → 差 26 年，是别的录制
 * 正例（真重映，年份对得上）：
 *   EVA 死與新生  豆瓣 1997 / tt0169880:1997 → 差 0
 *   追擊8月15    豆瓣 2004 / tt0441565:2004 → 差 0
 *
 * 候选无年份时不否决（IMDb 有些条目缺年份，不该因此丢掉真重映）。
 */
export function matchesDoubanYear(candYear, doubanYear) {
  if (!doubanYear) return false;
  if (candYear == null) return true;
